...
export interface SessionUser {
  ...
  authMethod?: string;
  bio?: string;
  role?: 'admin' | 'user' | 'moderator' | 'ADMIN' | 'USER' | 'MODERATOR'
}
...
export function setSession(data: SessionTokens): void {
  ...
  if (data.user) {
    // Normaliser le rôle en minuscules (backend envoie 'ADMIN'/'USER', frontend attend 'admin'/'user')
    const normalizedUser = {
      ...data.user,
      role: data.user.role ? (data.user.role.toLowerCase() as SessionUser['role']) : 'user'
    }
    localStorage.setItem(KEY_USER, JSON.stringify(normalizedUser))
  }
...
/** Logout: removes tokens and user */
...
export function hasSession(): boolean {
  ...
  if (!expiresAt) {
    console.log("[DEBUG] ExpiresAt manquant/null => clearSession appelé")
    clearSession()
    return false
  }
  ...
  if (!Number.isFinite(expirationTime)) {
    console.log("[DEBUG] ExpirationTime invalide => clearSession appelé")
    clearSession()
    return false
  }
...
/**
 * Logout on server side (session revocation) then local cleanup.
 * Calls POST /auth/logout with Bearer; in case of error (e.g. 401), still cleans up the client.
 */
